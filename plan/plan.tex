...
The optimization paradigm utilizes 
    ...
    $y_i$ are dual variables.

ADMM obtains the solutions for $x$ and $z$ via
    \EE{}{  x_i &\gets \argmin_{x_i} \Lag(x, z, y), \quad i = 1,\ldots,N \\
            z &\gets \argmin_{z} \Lag(x, z, y), \\
            y_i &\gets y_i + \rho (A_i(x_i) z - b(x_i)), \quad i = 1,\ldots,N.}
...
To obtain the correct $Q(z)$ which should be passed to the structure submodule,
    ...
    \E{}{\argmin_z \Lag = \argmin_z g(z) 
            + \sum_i^N \frac{\rho}{2} \norm{B_i(x_i) z - d(x_i)}^2 
            + \real\left[ y_i^\cT ( B_i(x_i) z - d_i(x_i))\right]}
    which means that
...
Updating $x_i$, which is left to the paradigm submodule,
    involves solving $\argmin_{x_i} \Lag(x, z, y)$ for all $i = 1, \ldots, N$.
This can be accomplished via
    \E{}{\grad_{x_i}\Lag = \grad_{x_i} f_i(x_i) + }
